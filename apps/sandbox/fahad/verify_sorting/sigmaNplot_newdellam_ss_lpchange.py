...
u_1 = np.loadtxt(
    r'C:\Users\fseemab\bmcs-master\apps\sandbox\fahad\sigNm0lp-20000(newdellam).txt')
u = np.transpose(u_1)
...
ax1.legend()
plt.title('m = 0 lp = -20000 (SS new del lam)')

ax2 = plt.subplot(222)

u_2 = np.loadtxt(
   r'C:\Users\fseemab\bmcs-master\apps\sandbox\fahad\sigNm01lp-20000(newdellam).txt')
u = np.transpose(u_2)
ln = np.linspace(0, 80, 5)
...
ax2.legend()
plt.title('m = 0.1 lp = -20000 (SS new del lam)')

ax3 = plt.subplot(223)

u_3 = np.loadtxt(
    r'C:\Users\fseemab\bmcs-master\apps\sandbox\fahad\sigNm05lp-20000(newdellam).txt')
u = np.transpose(u_3)
ln = np.linspace(0, 80, 5)
...
ax3.legend()
plt.title(' m = 0.5 lp = -20000 (SS new del lam)')
