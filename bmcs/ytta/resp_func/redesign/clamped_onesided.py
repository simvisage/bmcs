...
from enthought.traits.api import \
    HasTraits, Float, Str, implements, Range, Property, cached_property
...
class PullOutWithConstantFriction( RF ):
    ...
    def __call__( self, u, tau, l, E_f, D_f, cs ):
        ...
        q = q * H( T * ( d - l ) - q ) + ( E_f * A_f * ( u - u0 ) / d + T * ( d - l ) ) * H( q - ( d - l ) * T )
...
class StressInFiberWithConstantFriction( PullOutWithConstantFriction ):
    ...
    def __call__( self, x, u, tau, l, E_f, D_f ):
        ...
        P_f = pi * self.D_f
        T = tau * P_f
        q = super( StressInFiberWithConstantFriction, self ).__call__( u, tau, l, E_f, D_f )
        xi = x - l
